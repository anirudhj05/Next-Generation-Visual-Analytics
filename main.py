        ...
        if apply_button:
            ...
            if not cleaning_options:
                ...
                if total_missing > 0:
                    ...
                    st.markdown(
                        f"- Rows: {original_rows} → {final_rows} ({original_rows - final_rows} removed)"
                    )
                    st.markdown(
                        f"- Columns: {original_cols} → {final_cols} ({original_cols - final_cols} removed)"
                    )
                    st.markdown(
                        f"- Missing values: {original_missing} → {final_missing} ({original_missing - final_missing} filled or removed)"
                    )
                            ...
                            with tempfile.TemporaryDirectory() as tmpdir:
                                ...
                                if os.path.exists(organ_mask_path):
                                    ...
                                    with open(segmented_mask_path, "rb") as f:
                                        st.download_button(
                                            label=
                                            f"📥 Download {selected_organ} Segmentation (.nii.gz)",
                                            data=f,
                                            file_name=
